...
func CreateOrganization(org Organization) (int64, error) {
	query := "INSERT INTO organizations (name, slug, contact_email) VALUES (?, ?, ?)"

	if currentDriver == "postgres" {
		var id int64
		err := DB.QueryRow(Rebind(query+" RETURNING id"), org.Name, org.Slug, org.ContactEmail).Scan(&id)
		if err != nil {
	...
	res, err := DB.Exec(query, org.Name, org.Slug, org.ContactEmail)
	if err != nil {
...
func GetOrganization(id int64) (*Organization, error) {
	...
	err := DB.QueryRow(Rebind("SELECT id, name, slug, contact_email, created_at FROM organizations WHERE id = ?"), id).
...
func GetAllOrganizations() ([]Organization, error) {
	rows, err := DB.Query(Rebind("SELECT id, name, slug, contact_email, created_at FROM organizations ORDER BY name ASC"))
	if err != nil {
...
func UpdateOrganization(org Organization) error {
	_, err := DB.Exec(Rebind("UPDATE organizations SET name = ?, slug = ?, contact_email = ? WHERE id = ?"), org.Name, org.Slug, org.ContactEmail, org.ID)
	return err
}
